function [data_high,pks_frame,pks] = findActiveFramesCustom(data,pks)
...
% modified bin intervals and also fixed indexing error in original on
% line 44
...
if isempty(pks)    
    ...
    for n = 3:max(sum(data,1))
        ...
        warning('off')
        for ii = 1:num_shuff
            data_high_rnd = data_shuff(:,sum(data_shuff(:,:,ii))>=n,ii);
            S_rd = 1-pdist2(data_high_rnd',data_high_rnd','cosine');
            S_out(ii) = nanmean(S_rd(:));
        end
        warning('on')

        % determine threshold (custom adjusments)
        bins = 0:0.002:max(S_out)+0.002;
        cd = histc(S_out,bins);
        cd = cumsum(cd/sum(cd));
